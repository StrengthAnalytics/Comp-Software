...
- **Public** (`/(public)`): comp landing pages, a public **live scoreboard** (planned, at `/[comp]/live`) for venue TVs and social shares, final results, the **warm-up board** (`/[comp]/warm-up`) — a sign-in-free read-only mirror of the run-screen scoresheet for the warm-up room and for sharing with lifters/spectators — the **entry form** (`/[comp]/enter`) — lifters self-register into the comp's review inbox; one of the two public surfaces that *write* (a single INSERT into `entry_submissions`, see §3; the **volunteer staff rota** at `/[comp]/volunteer`, where volunteers claim staffing slots, is the other — a single INSERT into `rota_signups`) — and the **UK records browser** (`/records`), an app-global, sign-in-free table of regional/national records (not tied to any comp). No auth gate; comp-scoped anon reads are gated by RLS to publicly-visible comps with lifter names from the PII-free `public_lifters` view, while the records browser reads the always-public `records` table (see §3 and the ADR in §7).
...
- **sessions**: a chunk of lifting tied to a date and platform, with a weigh-in-open time (`weigh_in_time`) and a lift-off time (`lift_off_time`) — both informational (shown on the session form and echoed into the rota's generated column subtitles), neither wired into scoring or running order.
...
- **rota_sections**: (volunteer staff rota) a column of the rota grid for a comp — an optional `day_label` banner (e.g. "Sat"), a `title` (e.g. "AM", "Set-up"), an optional free-text `subtitle` (weigh-in / lift-off times), and `sort_order`. Admin-built. An optional `session_id` links a column to the comp session it was **generated** from (nullable — manual columns like Set-up carry none; `on delete set null`, with a partial unique index so a session has at most one column), which makes the "generate from sessions" action idempotent.
...
**The structure is its own small hierarchy, independent of the comp's sessions/flights.** `rota_sections` are the grid columns (e.g. "Sat — AM", "Set-up"), `rota_roles` the jobs within a section (title, arrive-by, and a slot `capacity`), and `rota_signups` a volunteer in a slot. It is deliberately *not* derived from the comp's `sessions`/`flights`: set-up and take-down are not sessions, the rota's groupings don't map one-to-one to flights, and coupling the two would force the rota to change whenever the running order does. `competition_id` is denormalised onto roles and sign-ups (as `attempts` carry it alongside `entry_id`) so the RLS predicate and realtime filter need no join. As a convenience a one-click **Generate from sessions** still creates a column per session, pre-filled with a default role template, via a nullable `rota_sections.session_id` link — idempotent (only sessions without a column are added) and `on delete set null` (a deleted session leaves its column). But a generated column is a normal, fully-editable section, **not a live mirror**, so the rota and the running order still never force changes on each other.
