...
- **Public** (`/(public)`): comp landing pages, live scoreboard for venue TVs and social shares, final results, and the **public warm-up board** (`/[comp]/warm-up-public`) — a sign-in-free copy of the Display warm-up board for sharing with lifters/spectators. Read-only, no auth gate; anon reads are scoped by RLS to publicly-visible comps and lifter names come from the PII-free `public_lifters` view.
