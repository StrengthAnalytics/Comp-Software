...
- **Public** (`/(public)`): comp landing pages, final results, a public **live scoreboard** (planned, at `/[comp]/standings`) for venue TVs and social shares, and the **public live warm-up board** (`/[comp]/live`) — a sign-in-free copy of the Display warm-up board, the comp's primary public live view, for sharing with lifters/spectators. Read-only, no auth gate; anon reads are scoped by RLS to publicly-visible comps and lifter names come from the PII-free `public_lifters` view.
