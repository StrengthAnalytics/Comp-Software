...
// gating, ends_on for the Overview page's date range) need, so every caller reads from the same
// memoized result.
export const getCompBySlug = cache(async (slug: string) => {
  const supabase = await createClient();
  const { data } = await supabase
    .from('competitions')
    .select('id, name, slug, event_type, status, is_team_competition, kit_type, starts_on, ends_on')
    .eq('slug', slug)
    .maybeSingle();
  return data;
});
