...
// kit_type for the run screen's IPF GL column) need, so every caller reads from the same memoized
// result.
export const getCompBySlug = cache(async (slug: string) => {
  const supabase = await createClient();
  const { data } = await supabase
    .from('competitions')
    .select('id, name, slug, event_type, status, is_team_competition, kit_type')
    .eq('slug', slug)
    .maybeSingle();
  return data;
});
