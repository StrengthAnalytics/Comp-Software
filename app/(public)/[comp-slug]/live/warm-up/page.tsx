...
// The public, sign-in-free warm-up room board. Identical to the admin `/warm-up` display — the
// read-only run scoresheet mirror, the up-next cards and the per-platform `?platform=` scoping — but
// it lives in the (public) route group (no admin gate) and loads its snapshot with `publicView`, so
// lifter names come from the PII-free `public_lifters` view rather than the admin-only base table.
// Everything else is read straight from the tables' anon read policies, which only return rows for a
// publicly-visible comp; hence the published-status guard below.
  ...
  if (!selected) {
    return (
      <DisplayPlatformChooser
        ...
        hrefForPlatform={(platformId) => `/${comp.slug}/live/warm-up?platform=${platformId}`}
      />
    );
  }
